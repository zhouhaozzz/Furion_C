...
wav_Lambda = bragg_Str.wave_Lambda*1e10;                           % [A]波长
...
data_f0_co = textread([path_Str.path_formfactor,file_f0_co],...
             '%s','delimiter', ' ','headerlines', 31);             % 读取文件，
index_atom = find(strcmp(data_f0_co, atomic_Sym));                 % 原子序号
index_atom = index_atom(1);                                        % 
data_f0_Co = data_f0_co((index_atom+3):(index_atom+11));           %读取系数
Data_f0_Co = str2num(char(data_f0_Co));                            %文本转换为数值数组
for k = 1:4
